...
var
	...
	WORLDREPORT_ITEMSIZE = 15, // how many float values each reported item needs
	worldreport,
...
createShape = function( description ) {
	...
	switch ( description.type ) {
		...
		case 'heightfield':
			...
			for (i = 0; i < description.points.length; i++) {
				Ammo.setValue(ptr + i,  description.points[i]  , 'float');
			}
...
public_functions.addObject = function( description ) {
	...
	if ( description.materialId !== undefined ) {
		if (_materials[description.materialId].restitution !== undefined) {
			rbInfo.set_m_restitution(_materials[description.materialId].restitution);
		}
		if (_materials[description.materialId].friction !== undefined) {
			rbInfo.set_m_friction(_materials[ description.materialId ].friction);
		}
		if (_materials[description.materialId].rollingFriction !== undefined) {
			rbInfo.set_m_rollingFriction(_materials[description.materialId].rollingFriction);
		}
		if (_materials[description.materialId].linearDamping !== undefined) {
			rbInfo.set_m_linearDamping(_materials[description.materialId].linearDamping);
		}
		if (_materials[description.materialId].angularDamping !== undefined) {
			rbInfo.set_m_angularDamping(_materials[description.materialId].angularDamping);
		}
		if (_materials[description.materialId].linearSleepingThreshold !== undefined) {
			rbInfo.set_m_linearSleepingThreshold(_materials[description.materialId].linearSleepingThreshold);
		}
		if (_materials[description.materialId].angularSleepingThreshold !== undefined) {
			rbInfo.set_m_angularSleepingThreshold(_materials[description.materialId].angularSleepingThreshold);
		}
		if (_materials[description.materialId].additionalDamping !== undefined) {
			rbInfo.set_m_additionalDamping(_materials[description.materialId].additionalDamping);
		}
		if (_materials[description.materialId].additionalDampingFactor !== undefined) {
			rbInfo.set_m_additionalDampingFactor(_materials[description.materialId].additionalDampingFactor);
		}
		if (_materials[description.materialId].additionalLinearDampingThresholdSqr !== undefined) {
			rbInfo.set_m_additionalLinearDampingThresholdSqr(_materials[description.materialId].additionalLinearDampingThresholdSqr);
		}
		if (_materials[description.materialId].additionalAngularDampingFactor !== undefined) {
			rbInfo.set_m_additionalAngularDampingFactor(_materials[description.materialId].additionalAngularDampingFactor);
		}
		if (_materials[description.materialId].additionalAngularDampingThresholdSqr !== undefined) {
			rbInfo.set_m_additionalAngularDampingThresholdSqr(_materials[description.materialId].additionalAngularDampingThresholdSqr);
		}
	}
...
public_functions.applyTorqueImpulse = function (details) {

	_vec3_1.setX(details.torque_x);
	_vec3_1.setY(details.torque_y);
	_vec3_1.setZ(details.torque_z);

	_objects[details.id].applyTorqueImpulse(_vec3_1);
	_objects[details.id].activate();
};
...
public_functions.setSleepingThresholds = function ( details ) {
	_objects[details.id].setSleepingThresholds( details.linearSleepingThreshold, details.angularSleepingThreshold );
};
...
public_functions.setRollingFriction = function ( details ) {
	_objects[details.id].setRollingFriction( details.rollingFriction );
};

public_functions.setAnisotropicFriction = function (details) {
	_vec3_1.setX(details.anisotropicFriction_x);
	_vec3_1.setY(details.anisotropicFriction_y);
	_vec3_1.setZ(details.anisotropicFriction_z);
	_objects[details.id].setAnisotropicFriction(_vec3_1, details.anisotropicFrictionFlags);
};

public_functions.setSplitImpulse = function (solverInfo) {
	var solver = world.getSolverInfo();

	solver.set_m_splitImpulse(solverInfo.splitImpulse);
	solver.set_m_splitImpulsePenetrationThreshold(solverInfo.splitImpulsePenetrationThreshold);
};
...
reportWorld = function() {
	...
	for ( index in _objects ) {
		if ( _objects.hasOwnProperty( index ) ) {
			...
			worldreport[ offset + 14 ] = object.isActive();
...
reportCollisions = function() {
	var i, offset,
		...
		numManifolds = dp.getNumManifolds(),
		manifold, numContacts, pt;

	if (SUPPORT_TRANSFERABLE) {
		// check our existing transfer buffer is going to be big enough
		// TODO: this needs to be 2 + (numContacts * SIZE) there are more contacts than manifolds??
		//if (collisionreport.length < 2 + numManifolds * COLLISIONREPORT_ITEMSIZE) {

		// when postMessage is called the local copy is always nullified so we need to initialise each frame
	...
	// reset the buffer's reports "count" to 0
	collisionreport[1] = 0;
	...
	for ( i = 0; i < numManifolds; i++ ) {
		...
		for (var j = 0; j < numContacts; j++) {
			...
			// find the report offset from the buffer's existing count
