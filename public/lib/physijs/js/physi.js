window.Physijs = (function() {
	...
	var _Physijs = Physijs; // used for noConflict method
	...
	Physijs.createMaterial = function(material,
		friction, restitution, rollingFriction,
		linearDamping, angularDamping,
		linearSleepingThreshold, angularSleepingThreshold,
		additionalDamping,
		additionalDampingFactor, additionalLinearDampingThresholdSqr,
		additionalAngularDampingFactor, additionalAngularDampingThresholdSqr
	) {
	...
	Physijs.Scene.prototype._updateScene = function( data ) {
		...
		for ( i = 0; i < num_objects; i++ ) {
			...
			var wasActive = object._physijs.active;
			object._physijs.active = data [ offset + 14 ];

			if (wasActive !== object._physijs.active) {
	...
	Physijs.Scene.prototype._updateCollisions = function( data ) {
		...
		var i, offset;
		var object1, object2, object1Id, object2Id;
		var collisions = {}, normalOffsets = {};

		// parse the collision report to generate dictionaries of all collision pairs from both directions
		for ( i = 0; i < data[1]; i++ ) {
			offset = 2 + i * COLLISIONREPORT_ITEMSIZE;
			object1Id = data[ offset ];
			object2Id = data[ offset + 1 ];

			normalOffsets[object1Id + '-' + object2Id] = offset + 2;
			normalOffsets[object2Id + '-' + object1Id] = -1 * (offset + 2);

			// Register collisions for both the object colliding and the object being collided with
			if (!collisions[object1Id]) collisions[object1Id] = [];
			collisions[object1Id].push(object2Id);

			if (!collisions[object2Id]) collisions[object2Id] = [];
			collisions[object2Id].push(object1Id);
		}

		// enumerate all of the dynamic objects
		for ( object1Id in this._objects ) {
			if (!this._objects.hasOwnProperty(object1Id)) continue;

			object1 = this._objects[object1Id];

			// process all existing "touches" caches from the previous frame for this object
			for (i = 0; i < object1._physijs.touches.length; i++) {

				object2Id = object1._physijs.touches[i];

				// if there are no collisions for object1 at all, or there is no collision with object2Id
				if (collisions[object1Id] == undefined || collisions[object1Id].indexOf(object2Id) === -1) {

					object2 = this._objects[object2Id];

					// remove object2Id from this touches cache (and decrement our indexer)
					object1._physijs.touches.splice(i--, 1);

					object1.dispatchEvent(
						'collisionStopped',
						{
							object1: object1,
							object2: object2
			...
			if (collisions[object1Id]) {

				// Handle each colliding object
				for (i = 0; i < collisions[object1Id].length; i++) {
					object2Id = collisions[object1Id][i];
					object2 = this._objects[object2Id];

					if (!object2) continue;

					// If object was not already touching object2, notify object
					if (object1._physijs.touches.indexOf(object2Id) === -1) {

						object1._physijs.touches.push(object2Id);
						...
						var normal_offset = normalOffsets[object1._physijs.id + '-' + object2._physijs.id];
						if (normal_offset > 0) {
							_temp_vector3_1.set(
								-data[normal_offset],
						...
						object1.dispatchEvent(
							'collision',
							{
								object1: object1,
								object2: object2,
								linearVelocityDelta: _temp1,
								angularVelocityDelta: _temp2,
								normalOffset: _temp_vector3_1
							}
						);
	...
	Physijs.Scene.prototype.setSplitImpulse = function (splitImpulse, splitImpulsePenetrationThreshold) {
		this.execute('setSplitImpulse', { splitImpulse: splitImpulse, splitImpulsePenetrationThreshold: splitImpulsePenetrationThreshold });
	};
	...
	// Physijs.Mesh.isActive
	Physijs.Mesh.prototype.isActive = function () {
		return this._physijs.active;
	};

	// Physijs.Mesh.isSleeping
	Physijs.Mesh.prototype.isSleeping = function () {
		return !this._physijs.active;
	};
	...
	// Physijs.Mesh.applyTorqueImpulse
	Physijs.Mesh.prototype.applyTorqueImpulse = function (force) {
		if ( this.world ) {
			this.world.execute( 'applyTorqueImpulse', { id: this._physijs.id, torque_x: force.x, torque_y: force.y, torque_z: force.z } );
		}
	};
	...
	// Physijs.Mesh.setSleepingThresholds
	Physijs.Mesh.prototype.setSleepingThresholds = function (linear, angular) {
		if (this.world) {
			this.world.execute('setSleepingThresholds', { id: this._physijs.id, linearSleepingThreshold: linear, angularSleepingThreshold: angular });
		}
	};
	...
	// Physijs.Mesh.setRollingFriction
	Physijs.Mesh.prototype.setRollingFriction = function (friction) {
		if (this.world) {
			this.world.execute('setRollingFriction', { id: this._physijs.id, rollingFriction: friction });
		}
	};

	// Physijs.Mesh.setAnisotropicFriction
	Physijs.Mesh.prototype.setAnisotropicFriction = function (anisotropicFriction, anisotropicFrictionFlags) {
		if (this.world) {
			this.world.execute('setAnisotropicFriction', {
				id: this._physijs.id,
				anisotropicFriction_x: anisotropicFriction.x,
				anisotropicFriction_y: anisotropicFriction.y,
				anisotropicFriction_z: anisotropicFriction.z,
				anisotropicFrictionFlags: anisotropicFrictionFlags
			});
		}
	};
	...
	// Physijs.BoundingBoxMesh
	Physijs.BoundingBoxMesh = function (geometry, material, mass) {
	...
	Physijs.BoundingBoxMesh.prototype = new Physijs.Mesh;
	Physijs.BoundingBoxMesh.prototype.constructor = Physijs.BoundingBoxMesh;
